...
SELECT v.*,
	   vb.market_date
FROM vendor AS v
INNER JOIN vendor_booth_assignments AS vb ON v.vendor_id = vb.vendor_id
...
SELECT vendor_id, 
	   COUNT(*) AS booth_assignments_count
...
SELECT  cp.customer_id, c.customer_last_name, c.customer_first_name,
		sum(cp.quantity * cp.cost_to_customer_per_qty) as [custome-money-spent]
